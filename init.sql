...
INSERT INTO control (id, number, language, name, description) VALUES (17, 10, 'en', 'Ensure the log destinations are set correctly', 'PostgreSQL supports several methods for logging server messages, including stderr, csvlog and syslog. On Windows, eventlog is also supported. One or more of these destinations should be set for server log output. If log_destination is not set, then any log messages generated by the core PostgreSQL processes will be lost.');
INSERT INTO control (id, number, language, name, description) VALUES (18, 10, 'ru', 'Назначение для сообщений журнала должно быть установлено правильно', 'PostgreSQL поддерживает несколько методов для журналирования сообщений сервера, включая stderr, csvlog и syslog. На системах Windows, также поддерживается журнал событий. Если log_destination не установлен, тогда любые сообщения журналов создаваемые процессами PostgreSQL будут потеряны.');
